...
func examplesCheckRoleRouter(r *gin.Engine, authMiddleware *jwtauth.GinJWTMiddleware) {
	...
	// {{认证路由自动补充在此处请勿删除}}
	registerAreaRouter(v1, authMiddleware)
	registerArticleRouter(v1, authMiddleware)
	registerInvestRouter(v1, authMiddleware)
	registerBorrowDiyaRouter(v1, authMiddleware)
	registerBorrowRouter(v1, authMiddleware)
	registerMembersRouter(v1, authMiddleware)
}
